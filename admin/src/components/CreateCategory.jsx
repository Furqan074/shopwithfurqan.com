...
function CreateCategory() {
  document.title = "Create Category | Shopwithfurqan";
  ...
  const handleCreateCategory = async (event) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/admin/categories/create`,
        {
          method: "POST",
          headers: {
      ...
      if (response.status === 403 || response.status === 400) {
        ...
        toast({
          variant: "destructive",
          description:
            "Unexpected Error occur! Category not created, contact your developer if it persists.",
        });
      }
      if (data.success) {
        toast({
