...
function CreateBanner() {
  ...
  const handleCreateBanner = async (event) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_BACKEND_URL}/admin/banners/create`,
        {
          method: "POST",
          headers: {
      ...
      if (response.status === 403 || response.status === 400) {
        ...
        toast({
          variant: "destructive",
          description:
            "Unexpected Error occurred! Banner not created, contact your developer if it persists.",
        });
      }
    } catch (error) {
  ...
  return (
    <form className="flex flex-col gap-4" onSubmit={handleCreateBanner}>
